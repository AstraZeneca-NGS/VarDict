...
while( <> ) {
    ...
    if ( @d1 ) {
        ...
        #foreach( @d2 ) { print STDERR join("\t", @$_), "\n"; }
...
foreach my $d (@data) {
    ...
    my ($af, $pmean, $qmean, $mq, $sn) = @$d[$HDRN{ AlleleFreq }, $HDRN{ Pmean }, $HDRN{ Qual }, $HDRN{ MQ }, $HDRN{ SN }];
    my ($m_af, $m_pmean, $m_qmean, $m_mq, $m_sn) = @$d[$HDRN{ Matched_AlleleFreq }, $HDRN{ Matched_Pmean }, $HDRN{ Matched_Qual }, $HDRN{ Matched_MQ }, $HDRN{ Matched_SN }] if ( @pairhdrs > 0);
    my $varn = @{ $var{ $vark } } + 0;
